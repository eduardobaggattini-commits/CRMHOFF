...
import { importarPlanilha, type ResultadoImportacao } from "@/app/(app)/clientes/importar/actions";
    ...
    <div className="space-y-6">
      <div className="rounded-2xl bg-white p-6 shadow">
        ...
        <ol className="mb-4 list-inside list-decimal space-y-1 text-sm text-slate-600">
          ...
          <li>Preencha as colunas (nome é obrigatório; o resto é opcional).</li>
        ...
        <p className="mt-3 text-xs text-slate-400">
          Colunas: nome, documento, telefone, email, cidade, estado, endereco, vendedor_email,
          observacoes
        </p>
      </div>
      ...
      {resultado && (
        <div className="rounded-2xl bg-white p-6 shadow">
          ...
          <p className="flex items-center gap-1.5 text-sm font-medium text-emerald-600">
            ...
            {resultado.sucesso} cliente(s) importado(s)/atualizado(s) com sucesso
          </p>
          {resultado.avisos.length > 0 && (
            <div className="mt-3">
              <p className="flex items-center gap-1.5 text-sm font-medium text-amber-700">
                <AlertTriangle className="h-4 w-4" />
                {resultado.avisos.length} linha(s) com aviso
              </p>
